...
export default function Home() {
	...
	const handleSubmit = async (event: FormEvent) => {
		...
		try {
			// FIX 1 & 2: Corrected the URL to port 3000 and path /chat
			const response = await fetch("http://localhost:3000/chat", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
				// FIX 3: Changed 'message' to 'question' to match the backend
				body: JSON.stringify({
			...
			// FIX 4: Changed 'data.reply' to 'data.answer' to match the backend
